...
import { MapPin, Phone, User, Users, CheckCircle2, ArrowRight } from 'lucide-react';
...
export function KafilahSection() {
  ...
  return (
    <section id="kafilah" className="bg-stone-50 text-neutral-900 py-12 border-b border-neutral-300 scroll-mt-14">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 sm:mb-8">
          ...
          <div>
            <Link
              href="/kafilah"
              className="inline-flex items-center gap-2 px-4 py-2 bg-white hover:bg-neutral-100 text-neutral-900 text-xs font-semibold rounded-md border border-neutral-300 transition-colors shadow-xs"
            >
        ...
        {/* 11 Villages Grid: Rectangular cards */}
        {loading ? (
          <div className="p-8 text-center bg-white border border-neutral-300 rounded-md text-xs text-neutral-500 shadow-xs">
            Memuat profil 11 kafilah desa...
          </div>
        ) : error ? (
          <div className="p-8 text-center bg-white border border-rose-200 rounded-md text-xs text-rose-600 shadow-xs">
            {error}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {kafilahList.map((k) => {
              ...
              return (
                <div
                  key={k.code}
                  className={`bg-white rounded-md border p-4 sm:p-5 transition-all flex flex-col justify-between shadow-xs ${
                    isMahatoHost
                ...
                >
                  <div className="space-y-2.5">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {isMahatoHost && (
                        <span className="px-2 py-0.5 bg-amber-100 text-amber-900 font-bold text-[10px] rounded-sm border border-amber-300 flex-shrink-0">
                          Tuan Rumah
                        </span>
                      )}
                    </div>
