...
export function BranchesSection() {
  ...
  return (
    <section id="cabang" className="bg-white text-neutral-900 py-12 border-b border-neutral-300 scroll-mt-14">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {/* Tab Buttons (Rectangular with subtle rounded corners) */}
        <div className="flex gap-2 overflow-x-auto pb-2 mb-6 border-b border-neutral-200 custom-scrollbar">
          {BRANCHES_DATA.map((b) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2.5 rounded-md text-xs sm:text-sm font-semibold whitespace-nowrap transition-colors flex items-center gap-2 border ${
                  isActive
              ...
              >
                ...
                <span
                  className={`text-[10px] px-1.5 py-0.2 rounded font-mono ${
                    isActive ? 'bg-emerald-900 text-emerald-200' : 'bg-neutral-200 text-neutral-600'
                  }`}
                >
                  {b.categories.length}
                </span>
        ...
        {/* Tab Detail Card: Rectangular with subtle rounded corners */}
        <div className="bg-stone-50 border border-neutral-300 rounded-md p-5 sm:p-6 space-y-6 shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-4 border-b border-neutral-200">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-amber-600 font-bold">۞</span>
                <h3 className="text-lg sm:text-xl font-bold text-neutral-900">{activeBranch.name}</h3>
              </div>
              <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-600 font-mono">
                <span className="px-2 py-0.5 bg-white border border-neutral-300 rounded-sm">Format: {activeBranch.format}</span>
                <span>•</span>
                <span className="px-2 py-0.5 bg-white border border-neutral-300 rounded-sm">Personel: {activeBranch.personel}</span>
              </div>
            </div>

            <span className="self-start sm:self-auto inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-100 border border-amber-300 rounded-md text-xs font-bold text-amber-900 shadow-xs">
              <span className="w-1.5 h-1.5 rounded-xs bg-amber-500" />
            ...
            <div className="lg:col-span-6 space-y-2.5">
              ...
              <div className="space-y-2">
                {activeBranch.categories.map((cat, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 bg-white border border-neutral-300 rounded-md text-xs hover:border-emerald-600 transition-colors shadow-xs"
                  >
                    <div className="flex items-center gap-2.5 font-medium text-neutral-900">
                      <span className="w-5 h-5 rounded-sm bg-emerald-800 text-white flex items-center justify-center text-[10px] font-mono font-bold flex-shrink-0">
                        {idx + 1}
                      </span>
                      <span className="font-semibold">{cat.name}</span>
                    </div>
                    <span className="font-mono text-[11px] font-semibold text-amber-900 bg-amber-50 px-2 py-0.5 rounded-sm border border-amber-200 flex-shrink-0">
                      {cat.ageLimit}
                    </span>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-1.5 text-xs text-neutral-700">
                  {activeBranch.ageRules.map((rule, idx) => (
                    <li key={idx} className="flex items-start gap-2 bg-white p-2.5 rounded-md border border-neutral-300 shadow-xs">
                      <div className="w-4 h-4 rounded-sm bg-emerald-100 text-emerald-700 flex items-center justify-center flex-shrink-0 mt-0.5">
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-1.5 text-xs text-neutral-700">
                  {activeBranch.specs.map((spec, idx) => (
                    <li key={idx} className="flex items-start gap-2 bg-white p-2.5 rounded-md border border-neutral-300 shadow-xs">
                      <span className="w-1.5 h-1.5 rounded-xs bg-amber-500 mt-1.5 flex-shrink-0" />
